...
OUTPUT_PATH = "/media/mateo/T7/dl_frames/"
...
def process(scrap_folder):
    try:
        name = scrap_folder.split("/")[-2] + "_" + scrap_folder.split("/")[-1]

        cmd = f"yolo predict model={weight} conf=0.2 source={scrap_folder} save=False save_txt save_conf name={name} verbose=False"
        print(f"* Command:\n{cmd}")
